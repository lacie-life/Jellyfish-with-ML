...
# Step 1: Train the discriminator with the real data and recognize it as real.
# Step 2: Train the discriminator with the fake data and recognize it as fake.
# Step 3: Train the generator with the fake data and recognize it as real.
...
for epoch in range(EPOCH_NUM):
    for i, data in enumerate(dataloader):
        ...
        netD.zero_grad()
        y_real = netD(x_real)
        loss_D_real = criterion(y_real, real_label.to(torch.float32))
        ...
        loss_D_fake = criterion(y_fake, fake_label.to(torch.float32))
        ...
        loss_G = criterion(y_fake_r, real_label.to(torch.float32))
        loss_G.backward()
        optimizerG.step()
